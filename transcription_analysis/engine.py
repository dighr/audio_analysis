...
def transcribe_any_audio(file_obj, language_code, type):
    ...
    if type == "google":
        ...
        from transcription_analysis import deepspeech_transcription
        ds = deepspeech_transcription.DeepSpeech()
        text = ds.transcribe(file_name)

    elif type == "watson":
        from transcription_analysis.watson_transcription import WatsonTranscription
        wt = WatsonTranscription()
        text = wt.transcribe(file_name)

    elif type == "azure":
        from transcription_analysis.azure_transcription import AzureTranscription
        at = AzureTranscription(lang=language_code)
